...
func (s *Service) GetAllCategories(ctx context.Context, dto *entitydto.IDRequest, page int, perPage int, isActive bool) ([]productcategorydto.CategoryDTO, int, error) {
	if categoryModels, total, err := s.r.GetAllCategories(ctx, dto.IDs, page, perPage, isActive); err != nil {
		return nil, 0, err
	} else {
...
func (s *Service) GetAllCategoriesMap(ctx context.Context, isActive bool, isAdditional, isComplement *bool) ([]productcategorydto.CategoryMapDTO, error) {
	if categoryModels, err := s.r.GetAllCategoriesMap(ctx, isActive, isAdditional, isComplement); err != nil {
		return nil, err
	} else {

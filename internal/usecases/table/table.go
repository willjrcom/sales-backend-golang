...
func (s *Service) GetAllTables(ctx context.Context, page, perPage int, isActive bool) ([]tabledto.TableDTO, int, error) {
	tableModels, count, err := s.r.GetAllTables(ctx, page, perPage, isActive)
	if err != nil {
		return nil, 0, err
	}

...
// ComplementItemsRequest filters for complement items sold.
type ComplementItemsRequest struct {
	Start time.Time `json:"start"`
	End   time.Time `json:"end"`
}

// ComplementItemsResponse holds name and quantity of complements.
type ComplementItemsResponse struct {
	Name     string  `json:"name"`
	Quantity float64 `json:"quantity"`
}

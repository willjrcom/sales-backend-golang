...
func loadCompanyModels(ctx context.Context, db *bun.DB) error {
	...
	db.RegisterModel((*productentity.CategoryAdditionalCategories)(nil))
	...
	if _, err := db.NewCreateTable().IfNotExists().Model((*productentity.CategoryAdditionalCategories)(nil)).Exec(ctx); err != nil {
		mu.Unlock()
		return err
	}

...
func (r *OrderTableRepositoryLocal) GetOrderTablesByTableId(ctx context.Context, tableId string, contact string) (tables []model.OrderTable, err error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	out := []model.OrderTable{}
	for _, t := range r.tables {
		if t.TableID.String() == tableId && t.Contact == contact {
			out = append(out, *t)
		}
	}
	return out, nil

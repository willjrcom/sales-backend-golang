...
func NewHandlerReport(s *reportusecases.Service) *handler.Handler {
	...
	r.Post("/orders-by-status", h.handleOrdersByStatus)
	r.Post("/avg-process-step-duration", h.handleAvgProcessStepDurationByRule)
	r.Post("/cancellation-rate", h.handleCancellationRate)
	r.Post("/current-queue-length", h.handleCurrentQueueLength)
	r.Post("/avg-delivery-time-by-driver", h.handleAvgDeliveryTimeByDriver)
	r.Post("/deliveries-per-driver", h.handleDeliveriesPerDriver)
	r.Post("/orders-per-table", h.handleOrdersPerTable)
	r.Post("/avg-queue-duration", h.handleAvgQueueDuration)
	r.Post("/avg-process-duration-by-product", h.handleAvgProcessDurationByProduct)
	r.Post("/total-queue-time-by-group-item", h.handleTotalQueueTimeByGroupItem)
	// Sales by shift
